...
### 坑 2：DateTime 字段的 filter 有特定规则（已在 CLI 里封装）

飞书 `records/search` 的 filter 对 DateTime 字段（type=5）：
- **支持** `is` / `isEmpty` / `isNotEmpty` / `isGreater` / `isLess`
- **不支持** `isGreaterEqual` / `isLessEqual` / `isNot` / `contains` / `doesNotContain`（会返回 `code=1254018 InvalidFilter`）
- `value` 必须是 `["ExactDate", "<毫秒字符串>"]` 二元数组；裸毫秒 int 也会触发 `1254018`
- `isGreater` / `isLess` 实际语义是**上海时区天粒度**下的 `>= day` / `< day`（毫秒中的时分秒部分被忽略）

**规避方案**：用语义日期参数（`records search` 专属）：
...
3. **优先把 `isGreater(start_ms) + isLess(end_ms)` 合并进 body.filter 走服务端**（单日 23s → 2.4s 实测）
4. 仅当用户 `--where` 顶层是 OR / 嵌套无法合并时，才回退扫全表 + 客户端过滤
5. 对结果跑 Link 展开

**不要手写 `--where '日期 > 1712000000000'`**。DSL 里不支持 DateTime 字段的范围算子（value 需要 `["ExactDate", ...]` 转换）；遇到这种写法 DSL 会抛 DslError。所有服务端 DateTime filter 都通过 `--date-*` 参数走。

### 坑 3：`--date-field` + OR filter 仍走客户端

如果你同时用 `--date-*` 和 `--where '... or ...'`（顶层 OR），飞书单层 filter 无法把两条 date AND 嵌入 OR 表达式，CLI 会 stderr 提示"回退客户端过滤"，然后扫全表。大表（10 万行+）在这条路径上仍然很慢。**尽量把 OR 展成 AND**，或者把日期查询拆开两步做。
